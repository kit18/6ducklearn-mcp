...
Do not use `codex mcp add` for manual setup because current Codex versions can begin OAuth before explicit scopes are supplied. Only if an existing `6ducklearn` entry points to a different URL or transport, remove it first with `codex mcp remove 6ducklearn`. Then add this block to `~/.codex/config.toml`:
